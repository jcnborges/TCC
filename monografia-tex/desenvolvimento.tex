...
dos sinais dos \emph{encoders} (os \emph{encoders} não haviam sido nem utilizados no projeto Bellator). Portanto,
...
valor é um byte, cuja faixa de variação é [0, 255].
	\item \textbf{ENCODER valor\_high valor\_low END\_CMD}: representa a leitura de cada \emph{encoder},
...
\begin{itemize}
	...
	\item Ajuste de velocidade: esta etapa consiste do ajuste dos níveis de PWM de cada roda, e utiliza
		para tal os valores de leitura dos \emph{encoders} e o setpoint do passo anterior.
		...
		a velocidade caso a contagem do \emph{encoder} esteja abaixo do \emph{setpoint} ou diminuindo caso esteja
		acima;
...
TS-7260 também, mas seu código não faz parte do \emph{software} desta placa (tslogic), por este motivo está
...
\label{tab:varsfuzzy}
\end{table}
As funções de pertinência sp(d), sm(d), sl(d), correspondentes a perto, médio e longe, são definitas pelos trapézios da figura \ref{fuzzygraph_des}:
