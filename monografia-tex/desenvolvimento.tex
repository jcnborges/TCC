...
\begin{itemize}
    \item[-] Reconstrução da Plataforma Robótica Bellator: Todos os passos realizados para reconstruir e adaptar a plataforma robótica para utilização no projeto.
    \item[-] Algoritmos de Navegação: Detalha a implementação dos algoritmos de navegação \emph{fuzzy} e FCM, de acordo com as respectivas revisões bibliográficas apresentadas no capítulo \ref{chap:fundteor}.
...
em 5cm, começando em 15cm até 115cm, totalizando 21 medidas. Os valores obtidos estão na tabela a
seguir:
...
da curva. O grau do polinômio interpolador escolhido foi o de grau 4, que aproximou-se bem da curva
real (medida), conforme é possível visualizar na figura~\ref{fig:curvasensores}, onde o eixo das abscissas
contém os valores de conversão, e o eixo das ordenadas a distância correspondente. Segue a equação
...
Com esta equação, bastou iterar pelos valores \emph{x} possíveis de conversão (0 a 255, pois a conversão
analógica/digital usa 8 bits) e montar a tabela para acessar com os valores de conversão e obter
a distância em centímetros. Esta tabela que mapeia os valores da conversão para a distância em centímetros
foi inserida no código da placa TS-7260, que será explicado em detalhes na seção~\ref{sec:softwarets}.
...
quando requeridos pela mesma, e somente na TS-7260 cada valor é utilizado para consultar a tabela, obtida
...
foi então movido para uma pasta compartilhada na rede. Esta pasta foi disponibilizada através de um
...
onde \mbox{192.168.0.3} era o IP local da máquina onde o software foi desenvolvido, \emph{/files}
...
\begin{itemize}
	\item Envio de um comando SYNC para o microcontrolador: isto faz com que o microcontrolador responda enviando
	...
	\item Leitura do pacote enviado pelo microcontrolador: o programa recebe os dados do microcontrolador e
		...
		cada sensor são então armazenados, assim como as leituras dos encoders;
	\item Execução do algoritmo de navegação: utiliza os dados armazenados para executar o algoritmo
		...
		O algoritmo de navegação é executado também na placa TS-7260, mas não faz parte do mesmo software;
	...
	\item Ajuste de velocidade: esta etapa consiste do ajuste dos níveis de PWM de cada roda, e utiliza
		...
		do setpoint estabelecido previamente. Este ajuste é apenas proporcional, simplesmente aumentando
		a velocidade caso a contagem do encoder esteja abaixo do \emph{setpoint} ou diminuindo caso esteja
		acima;
...
de navegação é executado. Vale lembrar que o algoritmo de navegação (Fuzzy ou FCM) executa na placa
TS-7260 também, mas seu código não faz parte do software desta placa (tslogic), por este motivo está
em um bloco separado no diagrama. As ações descritas no bloco C8051F340 são executadas no
microcontrolador.
...
Para elaborar o controle \emph{fuzzy} responsável pelo algoritmo de navegação, é necessário, primeiramente, definir as variáveis linguísticas e os conjuntos \emph{fuzzy} associados às mesmas. Posteriormente, deve-se definir as variáveis linguísticas de entrada e saída do sistema e as associar apropriadamente. Em seguida, é necessário definir o método de defuzzificação e, finalmente, as regras de inferência. Cada um destes passos será descrito em detalhes a seguir.

A biblioteca FLIE permite a definição de variáveis linguísticas na classe \emph{linguisticvariable}, que pode ser composta por diversos conjuntos \emph{fuzzy}, definidos a partir da classe \emph{category}. Cada \emph{category} é definida por uma faixa de valores válidos (\emph{range}), por 4 pontos que definem um trapézio com os intervalos de subida, máximo e descida do nível de pertinência do conjunto de entrada, para fuzzificação dos dados, e um nome apropriado, tal como Perto, Longe, Rápido, dependendo do contexto. Ou seja, cada valor de entrada é fuzzificado de acordo com seu nível de pertinência a cada categoria (conjunto \emph{fuzzy}) pertencente à variável linguística. A figura \ref{lingvar}, apresentada a seguir, demonstra de forma gráfica esta estrutura.
