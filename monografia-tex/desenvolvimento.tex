...
TS-7260, com o objetivo de guiar o robô em sua navegação, através da execução
dos algoritmos de navegação.
...
\begin{tikzpicture}[node distance = 2cm, auto]
	% Nós do flowchart
	\node [block] (opentty) {Configura serial};
	\node [block, below of=opentty] (sync) {Envia SYNC};
	\node [block, below of=sync] (read) {Leitura dos dados};
	\node [block, below of=read] (alg) {Executa algoritmo de navegação};
	\node [block, below of=alg] (adj) {Envia as ações};
	\node [block, below of=adj] (end) {Loop};
	% Ligações
	\path [line] (opentty)  -- (sync);
	\path [line] (sync) -- (read);
	\path [line] (read) -- (alg);
	\path [line] (alg) -- (adj);
	\path [line] (adj) -- (end);
...
\begin{equation}
\label{eq:sig_frente}
f(x)=\frac{1} {10+e^{-(10x+4,5)}}
\end{equation}

\begin{equation}
\label{eq:sig_tras}
f(x)=1-\frac{1} {10+e^{-(10x+4,5)}}
\end{equation}
...
Os conceitos foram interligados através das relações causais apresentadas na tabela \ref{tab:fcm-relacoes} e o valor numérico das intensidades está de acordo com a tabela \ref{tab:pesos}.
