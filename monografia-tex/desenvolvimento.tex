...
\subsection{Relações Causais e Eventos}
...
Tendo-se em vista a necessidade de uma resposta dinâmica que permitisse modificar o valor os pesos em tempo real, introduziu-se ao sistema um evento que determina a transição de estado do FCM. Foram definidos dois estados, que são: ``FRENTE"~ e ``TRÁS". Os pesos das relações causais do primeiro estado está de acordo com a tabela \ref{tab:fcm-relacoes-frente}, os pesos do segundo, com a tabela \ref{tab:fcm-relacoes-tras} e o valor numérico das intensidades das relações causais estão de acordo com a tabela \ref{tab:pesos}.

\begin{table}[htb!]
	\centering
	\caption[Relações causais do controlador FCM (Estado = Frente) proposto]{Relações causais do controlador FCM proposto (Estado = FRENTE).}		
...
\begin{table}[htb!]
	\centering
	\caption[Relações causais do controlador FCM (Estado = Trás) proposto]{Relações causais do controlador FCM proposto (Estado = TRÁS).}		
	\begin{tabular}[!htb]{ l l l l }
	  \hline
	  Relação causal & Descrição & Efeito & Intensidade \\
	  \hline
...
\begin{table}[htb!]
	\centering
	\caption[Valor dos limiares L0 e L1]{Valor dos limiares L0 e L1.}		
	\begin{tabular}[!htb]{ l l l l }
	  \hline
...
\begin{itemize}
\item SE (SF) menor que o limiar (L0) e Estado igual a FRENTE ENTÃO Estado muda para TRÁS.
\item SE (SF) maior que o limiar (L1) e Estado igual a TRÁS ENTÃO Estado muda para FRENTE.
\end{itemize}
