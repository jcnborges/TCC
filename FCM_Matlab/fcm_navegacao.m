...
variacaosensores(i)=(SensorDN(i)-SensorEN(i));
varSN(i)=variacaosensores(i)/max(variacaosensores);
...
% conceito 4 - aceleração ( aumentar ou diminuir a velocidade )    
 
%xx=(A5F(kk)*E8+A6F(kk)*E9);

%A4F(kk+1)=(1/(1+exp(-1.25*xx)));
 ...
 % if SensorDN>=0.8 % se o sensor da direita "enxergar" obejto com
 % intensidade muito forte virar a esquera !!!! 
 % if SendorEN>=0.8 % de maneira analoga o sensor da esquerda !!!!
...
% andar em linha reta !!!! 
    ...
    % pensar em um robo basico que tem limite de distancia de ação sensor e
 % tres sensores a esquerda direita e frontal ; 
 % fcm ira tomar decisoes dinamicas quanto a desvio de objetos. 
%  distancias serao conceitos de entrada 
% http://www.sparkfun.com/commerce/product_info.php?products_id=9211
...
if dist3<=11 % alcance do sensor (menos que isso nao capta nada)    
    ...
    if dy3(i)<0
    SE(i)= 1/(dx3(i)^2+dy3(i)^2)^0.5;   SEN(k)=SE(i)/max(SE);  % somatorio final
...
if  dist<=11
 if dx(i)<0    
     ...
     if dy(i)<0
         ...
         SD(i)= 1/(dx(i)^2+dy(i)^2)^0.5;  SDN(k)=SD(i)/max(SD);
...
for i=1:nz-6
    % dimensoes das juntas de acordo com a geometria do KANGURU PAPER
    shading interp
    ...
    % as juntas estao conectadas como pendulos simples
...
%%ultima etapa ajustar sensores e definir regras fuzzy discretizadas com a
%%entrada dos sensores, talvez o próprio desenvolvimento dinamico do robo
%%seja uma contribuição de desenvolvimento do
% AS REGRAS DE ACORDO COM UMA ESTRADA DISCRETIZADA IRA AJUSTAR O PESO DA
% RELAÇÃO CAUSAL DO MAPA REGRAS VIRAR A ESQUERDA, VIRAR A DIREITA, ACELERAR
% E OU ATE MESMO PARAR - TESTAR EM AMBIENTES DINAMICOS.
